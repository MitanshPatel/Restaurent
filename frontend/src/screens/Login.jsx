...
function Login() {
  ...
  const handleSubmit = async(event) =>{
    ...
    const res = await fetch("https://lacuisinemagique.onrender.com/api/login",{
      method:"POST",
      headers:{
    ...
    <div className={css.sign}>
      <div className={css.con}>
        <form className={css.formula} onSubmit={handleSubmit}>
          <h1 style={{"color":"#f7d702", "fontWeight":"bolder"}}>Login</h1>
          ...
          <Link to="/createuser" className={css.btnol}>Sign up</Link>
        </form>
      </div>
